...
class Trainer(BasicTrainer):
    ...
    def _setup_model(self):
        ...
        self.model = getattr(module, self.opt.model.model).build_model_from(self.opt, param_outfile, c_out=3)
        self.modelG = self.model
        module = import_module('models')
        print(f"Using discriminator model {self.opt.model.modelD}!")
        self.modelD = getattr(module, self.opt.model.modelD).build_model_from(self.opt, c_in=3)
    ...
    def _optimize(self):
        ...
        for i in range(self.opt.g_steps):
            ...
            p_recon, z = self.modelG(g_in.detach(), noise_factor=self.opt.model.noise_factor, fix_sample=True)
            fake_data = p_recon
        ...
        if self.iter_counter % self.opt.log_freq == 0:
            ...
            self.visuals = {'train_patch': V.tensor_to_visual(img_patch[:,:3]),\
                            ...
                            # 'train_patch_depth': V.tensor_to_visual(img_patch[:,-1].unsqueeze(1)),
                            'train_patch_recon': V.tensor_to_visual(p_recon[:,:3]), 
                            # 'train_patch_recon_depth': V.tensor_to_visual(p_recon[:,-1].unsqueeze(1)), 
                            'train_global_recon': V.tensor_to_visual(global_recon[:,:3]),     
                            # 'train_global_depth': V.tensor_to_visual(global_recon[:,-1].unsqueeze(1)),     
