...
class TextureImageDataset(Dataset):
    ...
    def __init__(self, opt, octaves=1, scale_factor=0.25, transform_type="default"):
    ...
    def weighted_crop(self, x, upscale_factor=4):
        ...
        if self.default_h <= self.global_res:
            top = 0
        elif sign_t == 0:
            top = int((1 - exp_distribution(top, self.sample_sigma)) * upscale_factor * (self.default_h - self.global_res) / 2)
        else:
            top = int((exp_distribution(top, self.sample_sigma)+1) * upscale_factor * (self.default_h - self.global_res) / 2)
    ...
    def __getitem__(self, idx):
        ...
        if self.octaves <= 1:            
            ...
            if self.transform_type == 'positioned_crop':
                file_patch, d = self.weighted_crop(file_patch)
        else:
            file_patch = torch.stack([T(file) for T in self.transforms],0)

        if self.transform_type == 'bottleneck':
            file = self.transform_ref(file)
